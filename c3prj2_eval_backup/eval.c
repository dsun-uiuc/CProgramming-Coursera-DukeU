...
int card_ptr_comp(const void * vp1, const void * vp2) {
  ...
  return (cp1->value == cp2->value) ? (cp2->suit - cp1->suit) : (cp1->value - cp2->value);
}
...
size_t get_match_index(unsigned * match_counts, size_t n,unsigned n_of_akind){
  ...
  // for ( ; i < n; i++) {
    // if (match_counts[i] == n_of_akind) {
      ...
      // }
    // }
  while(match_counts[i] != n_of_akind) i++;
  return i;
}
...
hand_eval_t build_hand_from_match(deck_t * hand, unsigned n, hand_ranking_t what, size_t idx) {
  ...
  //  card_t *cards[5];}; typedef struct hand_eval_tag hand_eval_t;
  // typedef enum { STRAIGHT_FLUSH, FOUR_OF_A_KIND, FULL_HOUSE, FLUSH,
  // STRAIGHT, THREE_OF_A_KIND, TWO_PAIR, PAIR, NOTHING } hand_ranking_t;
